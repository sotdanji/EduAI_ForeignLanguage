...
class PreprocessorAgent(BaseGeminiAgent):
    ...
    def analyze_document_intent(self, input_data: Union[str, Any], input_type: str = "image") -> Dict[str, Any]:
        ...
        5. 다단(2단 이상) 편집 인식 [매우 중요]: 시험지나 문서가 2단(좌/우) 이상으로 나뉘어 있는 경우, 반드시 '왼쪽 단'의 내용을 위에서 아래로 끝까지 먼저 모두 읽어서 추출한 다음, 이어서 '오른쪽 단'의 내용을 읽으세요. 절대 왼쪽과 오른쪽을 가로질러(지그재그로) 읽어서 문장이 섞이게 하지 마세요.
        6. 박스(테두리) 지문 처리: 원본 이미지에서 네모난 선(테두리)이나 박스로 둘러싸인 지문/대화문이 있다면, 해당 텍스트 블록 위아래에 마크다운 인용구(`> `)를 붙이거나 `[박스 지문 시작]` / `[박스 지문 끝]` 기호를 명시적으로 추가하여 해당 내용이 박스 안에 있었음을 확실히 구분해 주세요.
